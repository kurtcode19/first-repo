...
frame_main.pack(expand=True, fill="both", padx=10, pady=(0, 20))  # <-- bottom padding 20
event_list = [
...
for event in event_list:
    ...
    frame.pack(padx=10, pady=5, fill='x')
        # Event name
    event_name = ctk.CTkLabel(frame, text=event['name'], font=('Arial', 14, 'bold'))
    event_name.pack(side=ctk.TOP, anchor='w')
    
    # Event details
    event_details = ctk.CTkLabel(frame, text=f"Date: {event['date']}\nLocation: {event['location']}")
